...
class PlanningManager {
    ...
    setupEventListeners() {
        ...
        if (currentWeekBtn) currentWeekBtn.addEventListener('click', () => this.goToCurrentWeek());
        ...
        // Fermeture des modales - Version améliorée
        ...
        if (addSlotForm) {
            addSlotForm.addEventListener('submit', (e) => this.handleAddSlotForm(e));
        }
    ...
    addSlot(employeeId, dateStr) {
        ...
        document.getElementById('addSlotPerson').value = employeeId;
        document.getElementById('addSlotDate').value = dateStr;
        
        // Remplir les listes déroulantes
        this.populateAddSlotModal();
        
        // Ouvrir la modale
        this.openModal('addSlotModal');
    }
    
    populateAddSlotModal() {
        // Remplir la liste des coachs
        const personSelect = document.getElementById('addSlotPerson');
        if (personSelect) {
            personSelect.innerHTML = '<option value="">Sélectionner un coach...</option>';
            this.employees.forEach(emp => {
                if (emp.active) {
        ...
        // Remplir la liste des bassins
        const locationSelect = document.getElementById('addSlotLocation');
        if (locationSelect) {
            locationSelect.innerHTML = '<option value="">Sélectionner un bassin...</option>';
        ...
        const startTimeInput = document.getElementById('addSlotStartTime');
        const endTimeInput = document.getElementById('addSlotEndTime');
    ...
    handleAddSlotForm(event) {
        event.preventDefault();
        
        const employeeId = document.getElementById('addSlotPerson').value;
        const dateStr = document.getElementById('addSlotDate').value;
        const startTime = document.getElementById('addSlotStartTime').value;
        const endTime = document.getElementById('addSlotEndTime').value;
        const locationId = document.getElementById('addSlotLocation').value;
        const courseType = document.getElementById('addSlotCourseType').value;
        const description = document.getElementById('addSlotDescription').value;
        
        // Validation
        if (!employeeId || !dateStr || !startTime || !endTime || !locationId || !courseType) {
        ...
        if (hasConflict) {
            this.showNotification('Conflit détecté : un créneau existe déjà à cette heure.', 'error');
            return;
        }
        ...
        const newSlot = {
            ...
            description: description || this.generateCourseDescription(courseType),
            participants: Math.floor(Math.random() * 12) + 3
        };
    ...
    goToCurrentWeek() {
        ...
        if (this.currentWeekStart.getTime() !== currentWeekStart.getTime()) {
            ...
            // Regénérer les données si nécessaire
            const weekDates = this.getWeekDays().map(day => day.fullDate.toISOString().split('T')[0]);
            let hasData = false;
            ...
            if (!hasData) {
                console.log('📅 Génération de données pour la semaine courante');
                this.generateWeekSchedule();
            }
            
            this.renderCurrentView();
            this.showNotification('Retour à la semaine courante', 'success');
        } else {
            this.showNotification('Vous êtes déjà sur la semaine courante', 'info');
        }
    }
